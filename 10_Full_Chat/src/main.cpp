...
static const char*        OLLAMA_MODEL  = "gemma3:12b";
static const int          SPEAKER_ID    = 8;   // 8 = 春日部つむぎ
...
bool synthesize(const String& text, uint8_t*& out_wav, size_t& out_len) {
  ...
  while (http2.connected() && read_total < (size_t)wav_len) {
    ...
    if (avail) { read_total += stream->readBytes(buf + read_total, avail); }
    else       { delay(1); }
  }
  http2.end();
...
// ---- WAV(24kHz/16bit/mono)を再生（章⑨と同じ） ----
